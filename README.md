...
\- `train\_example\_model.py`: trains RandomForest, evaluates metrics, serializes model

\- `load\_model.py`: safely loads serialized model for inference
...
\- `knowledge\_base.json`: small knowledge base for retrieval

\- `retriever.py`: keyword-based document retrieval

\- `llm\_client.py`: LLM wrapper for OpenAI/Azure/OpenAI-compatible APIs

\- `llm\_rag\_app.py`: RAG orchestration layer
...
\- `logging\_utils.py`: Loguru logger configuration with rotation
...
&nbsp;  - Invokes the sklearn model’s `predict\_proba`
...
&nbsp;  - Loads documents from `knowledge\_base.json`
...
uvicorn src.api.fastapi\_app:app --reload
...
\- `src/models/train\_example\_model.py`  

\- `src/models/load\_model.py`  
...
\- `src/generative/knowledge\_base.json`  

\- `src/generative/retriever.py`  

\- `src/generative/llm\_client.py`  

\- `src/generative/llm\_rag\_app.py`
...
OPENAI\_API\_KEY=<your-api-key-or-leave-empty>

MODEL\_PATH=src/models/model.pkl
...
If `OPENAI\_API\_KEY` is empty, the system will run with \*\*deterministic offline-safe LLM responses\*\*.
...
python -m src.models.train\_example\_model
...
\- Uses keyword-based retrieval over `knowledge\_base.json`
...
az acr build --registry <ACR\_NAME> --image ai-governance-demo:latest .
...
az containerapp create   --name ai-governance-demo   --resource-group <RG>   --image <ACR\_NAME>.azurecr.io/ai-governance-demo:latest   --target-port 8000   --ingress external
...
aws ecr get-login-password | docker login --username AWS --password-stdin <AWS\_ECR\_URI>



docker build -t ai-governance-demo .

docker tag ai-governance-demo:latest <AWS\_ECR\_URI>/ai-governance-demo:latest

docker push <AWS\_ECR\_URI>/ai-governance-demo:latest
...
&nbsp;gcloud builds submit --tag gcr.io/<PROJECT\_ID>/ai-governance-demo
...
gcloud run deploy ai-governance-demo   --image gcr.io/<PROJECT\_ID>/ai-governance-demo   --platform managed   --region us-central1   --allow-unauthenticated
...
&nbsp;   test\_api\_predict.py

&nbsp;   test\_api\_rag.py

&nbsp; model/

&nbsp;   test\_model\_loading.py

&nbsp;   test\_model\_accuracy.py
...
\*\*tests/unit/test\_retriever.py\*\*



```python

from src.generative.retriever import retrieve\_documents



def test\_retriever\_finds\_relevant\_docs():
...
&nbsp;   results = retrieve\_documents("governance", kb, top\_k=1)
...
\*\*tests/integration/test\_api\_predict.py\*\*
...
from src.api.fastapi\_app import app
